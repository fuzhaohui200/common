...
public class ShellsScriptThreadManage {

    private static Logger logger = LoggerFactory.getLogger(ShellsScriptThreadManage.class);
    ...
    public static Map<String, ServerShellBean> readConfigXMLInfo() {
        ...
        try {
            File file = new File(ShellsScriptThreadListener.TIVOLICONF_PATH);
    ...
    private static void moveShellScript(String folderName, File shellScriptFile, boolean flag) {
        try {
            String targetFolder;
            if (flag) {
                targetFolder = ShellsScriptThreadListener.SUCCES_FOLDER_PATH + "\\" + folderName.substring(folderName.lastIndexOf("\\") + 1);
            } else {
                targetFolder = ShellsScriptThreadListener.FAILED_FOLDER_PATH + "\\" + folderName.substring(folderName.lastIndexOf("\\") + 1);
            }
    ...
    private static void readShellScript(String folderName, File file) {
        ...
        // JobDetail excuteShellCommandJob = null;
        if (fileName.indexOf(".") != -1 && fileName.substring(fileName.indexOf(".")).equals(ShellsScriptThreadListener.SHELL_SUFFIX)) {
            ...
            try {
                ...
                List<ServerShellBean> serverShellList = ShellsScriptThreadManage.getAllServerInfo();
                for (ServerShellBean serverShellBean : serverShellList) {
                    if (serverShellBean.getName() != null && serverShellBean.getName().trim().equalsIgnoreCase(groupId.trim())) {
                        //logger.info("------- 正在执行脚本文件  ------------- " + groupId + " : "+ file.getName());
                        ...
                        for (Iterator<String> iterator = shellScriptFileMap.keySet().iterator(); iterator.hasNext(); ) {
                            ...
                            if (file.getName().equalsIgnoreCase(shFileName.trim())) {
                                logger.info("**********" + minitorName + "Excuting Thread ShellScriptFile: [" + shFileName + ", " + serverShellBean.getName() + "]**************");
                                // 为每个Shell脚本文件开一个线程
                                String ipAddress = serverShellBean.getIpAddress();
                                if (ipAddress != null && !ipAddress.equals("")) {
                                    ExecuteShellCommandThread excuteShellCommand = new ExecuteShellCommandThread(shFileName, serverShellBean.getName(), ShellsScriptThreadListener.WEBSERVICE_URL, serverShellBean.getIpAddress(),
                                            serverShellBean.getUsername(), serverShellBean.getPassword(), serverShellBean.getTelnetCmdPrompt());
                                    ThreadPoolExecuteTaskUtil.excuteTask(excuteShellCommand);
                                }
                            }
                        }
                    }
    ...
    private static void bakTivoliConfig() {
        String tivoliConfigPath = ShellsScriptThreadListener.TIVOLICONF_PATH;
